...
\paragraph{}
Some the of the keywords used to reveal sensitive information have already been hinted above. This is the list of our searches.

\begin{itemize}

\item \textbf{Password, pass or passwd.} We found a lot of instances of shared URLs which had the clear text passwords or just the hash of the password and could give us access to the account of the person sharing that URL. There were also a lot of password reset URLs, sometimes from the same websites. And quite a few shadowed passwords.

\item \textbf{Username.} The majority of the occurances were in combination with the above.

\item \textbf{Mail.} This returned multiple results of email addresses and accounts. The fact that people try to share the contents of personal emails became quite obvious.

\item \textbf{Admin or administrator.} Many results contained the word admin in the URL and sometimes providing access in parts of websites that are not meant to be seen and accessed by the public.

\item \textbf{Login.} Usually in combination with the above and it sometimes allowed us to see session IDs.

\item \textbf{Maps.} A large amount of data was Google Maps (and other internet maps) links that contained exact coordinates of a person's whereabouts. Some of them were used by third party applications that created a link when the user would "check in" somewhere, which if combined with the statistics that the URL shortening services provide, could reveal exactly when that person visited a particular place.

\item \textbf{Common service names.} We searched for keywords such as mailserver, webserver etc. Some of these services just linked directly to a login page.

\item \textbf{Ftp.} Many people might choose to share a file from their public ftp server. However when that server is not configured properly, one can gain access to other parts of it. When we searched for FTP servers, we were able to find exact IP addresses and ports that are used to access them. Also by just editing parts of the URL we could visit other parts of the servers.
...
\item \textbf{Network Gear Vendor Names.} We searched for keywords such as Cisco, Juniper etc. Some of the results we found were logfiles of certain routers after updates which could also lead back to an FTP server.

\item \textbf{Company/Bank/Institute Names} The same idea as above. We are glad to say that the majority of the results we found from banks did not contain sensitive information. We did witness some companies sharing internal links to systems that are solely aimed to their staff.
